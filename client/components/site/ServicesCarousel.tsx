...
export function ServicesCarousel({ items, intervalMs = 4500, options }: Props) {
  const opts: EmblaOptionsType = useMemo(() => ({ loop: true, align: "start", ...options }), [options]);
  ...
  return (
    <div className="relative" onMouseEnter={() => { setHovered(true); stop(); }} onMouseLeave={() => { setHovered(false); play(); }}>
      ...
      {/* Arrows */}
      <button aria-label="Previous" onClick={prev} className="absolute -left-2 top-1/2 -translate-y-1/2 hidden sm:inline-flex h-10 w-10 items-center justify-center rounded-full bg-white/80 text-[#0e0e0e] shadow hover:bg-white">
        <ChevronLeft className="h-5 w-5" />
      </button>
      <button aria-label="Next" onClick={next} className="absolute -right-2 top-1/2 -translate-y-1/2 hidden sm:inline-flex h-10 w-10 items-center justify-center rounded-full bg-white/80 text-[#0e0e0e] shadow hover:bg-white">
      ...
      <div className="mt-4 flex items-center justify-center gap-2">
        {items.map((_, i) => (
          <span key={i} className={`h-2 w-2 rounded-full ${selected === i ? "bg-[#5170FF]" : "bg-muted"}`} />
        ))}
      </div>
    </div>
  );
